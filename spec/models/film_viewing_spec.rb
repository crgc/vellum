...
RSpec.describe FilmViewing, type: :model do
  ...
  describe 'when it has a title, running time, and rating' do
    it 'is valid' do
      film_viewing_1.author_id = user.id
      film_viewing_1.save

      expect(film_viewing_1).to be_valid
    end
  end
  ...
  describe 'when it belongs to a group' do
    it 'is valid' do
      film_viewing_2.author_id = user.id
      film_viewing_2.group_ids = group.id
      film_viewing_2.save
      group.save

      expect(group.film_viewing_ids).to include(film_viewing_2.id)
    end
  end
